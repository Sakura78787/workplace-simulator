...
4. **发疯数值体系与判定 (Stats System)**
   ...
   * **隐藏结局机制（MVP）**：针对"发疯/摆烂/极端甩锅"等输入，在通过安全审查后可触发隐藏结局标记（如"主动离职流"、"彻底摆烂流"）。触发后依旧按效果值推进本回合，不中断主流程。
5. **结算海报与图鉴保存 (Result & Share)**
   ...
   * **隐藏结局展示（MVP）**：若触发隐藏结局，结算海报与图鉴元数据需记录并展示对应标签与触发语境摘要。
   * **称号映射基线（MVP）**：
     - Mental=0 -> "精神已读不回" / "确诊ICU的肝帝"
     - Shield=0 -> "孤狼·没有人站你" / "公敌·全员拉黑"
     - KPI=0 -> "大饼碎了一地" / "画饼翻车现场"
     - 熬过5回合 -> "职场端水大师" (均>60) / "比死还惨的幸存者" (均<=30)
   * **热力百分比算法基线（MVP）**：综合三项剩余数值计算"击败全国 x% 社畜"，其中精神值权重最高（0.5）。
6. **LLM 输入边界与服务降级 (MVP 强制项)**

   * **接入模型与时延策略**：MVP 接入 Opencode API（qwen3.5-plus）。请求发出后 3 秒内未返回时展示趣味 Loading 文案；5 秒仍未完成则硬切兜底。
   * **加载文案基线（MVP）**：LLM 请求超过 3 秒时轮播示例文案"老板正在打字..."、"你的回复正在被HR审查..."。
   * **降级提示文案基线（MVP）**：触发兜底时展示"职场服务器开小差了。不过别高兴，扣分照旧。"。
